...
import requests, platform, os, re, time, sys, json, random, datetime
from bs4 import BeautifulSoup as bsp
from concurrent.futures import ThreadPoolExecutor
#from method import ibrut_old as Bdt_old
import sysnotfound404 as Bdt_old
...
[white]Masukan Nama File Untuk Melihat Hasil, Gunakan koma untuk pemisahan nama file',True)
   ...
   def Exekusi(self, uid, cokie, next, mode,unli=None):
       ...
       params  = {'count': '100','search_surface': 'follow_list_page','max_id':next}
       if mode == 'following':response = requests.get(f'https://www.instagram.com/api/v1/friendships/{uid}/following/?count=100&max_id={next}', cookies=cokie,headers=headers).json()
